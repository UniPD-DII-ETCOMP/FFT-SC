function [vsol] = fun_resolve_Jac(x,FF,theta,tau,Ind,d,idxV,idxF,idxFx,idxFy,...
                            idxFz,Kt,Ae,Ae1x,Ae1y,Ae1z,L,M,N,...
                            circ_L,Aee, ...
                            K,prec_L,fl_precon_type)
%%
...
fMVM = @(J) multiplyMATVECT_EDDY_NEW5_JAC(J,theta,tau,...
                                        circ_L,z_realx_loc,z_realy_loc,...
                                        z_realz_loc,Hjac,...
                                        idxFx,idxFy,idxFz,d,Aee,L,M,N);
%% 
[Y_inv,LL,UU,PP,QQ,RR,~]=preparePREC_EDDY_JAC(theta,tau,d,...
                                        z_realF,idxFx,idxFy,idxFz,...
                                        prec_L,Hjac,Aee,Kt,fl_precon_type);
fPMV = @(JOut_full_in) multiplyPREC(JOut_full_in,Aee,Y_inv,LL,UU,PP,QQ,RR,fl_precon_type);
%% 
global NLcounter
if NLcounter==1
tol = 1e-4;
...
[vsol, flag, relres, iter, resvec]=pgmres_disp(@(J)fMVM(J),FF, inner_it, tol, outer_it,...
    @(JOut_full_in)fPMV(JOut_full_in) );
