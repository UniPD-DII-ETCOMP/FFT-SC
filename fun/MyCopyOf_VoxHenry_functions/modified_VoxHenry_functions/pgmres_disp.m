...
%
%
% MODIFIED BY F.L. ADDING FLAG_DISP for display results

function [x,flag,relres,iter,resvec] = pgmres_mod(A,b,restart,tol,maxit,L1,L2,R1,R2,x0,flag_disp)
...
if(nargin < 11 || isempty(flag_disp))
   flag_disp = 0; % no display default
end
...
while(resvec(it) > tol) && (outit < maxit)
   ...
   [x,r,p,resvec_inner] = pgmres_iter_disp(A,x,r,restart,L1,L2,R1,R2,tol*bnorm,flag_disp);
   ...
   if flag_disp
       if(it+p >= restart)
           disp(['   '])
           disp(['Achieved min norm of residual at each outer iter ::: ',...
               num2str(min(resvec(it+1:it+p)))])
           disp(['   '])
       end
   end
   it = it + p;
...
if flag_disp
    disp(['# of total/inner/outer iterations  ::: ', ...
        num2str(length(resvec)),' / ',num2str(iter(1)),' / ',num2str(iter(2))]);
    disp(['Achieved minimum residual norm NORM(B-A*X) ::: ',...
        num2str(min(resvec))])
end
